...
def get_fields(link):
    ...
    for item in data[1]:
        try:
            ...
            if _type == 7:
                for question in item[4]:
                    options = []
                    for option in question[1]:
                        options.append({"option": option[0], "type": question[-1][0]})
                    items.append({"id": "entry."+str(question[0]), "question": question[3][0], "type": _type, "req": req, "options": options})
